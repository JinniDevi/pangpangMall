	...
	<div class="untree_co-section">
		<div class="container d-flex flex-row justify-content-around">
			...
			<div class="col-lg-8 mt-4">
				...
				<article>
					<!-- Post header-->
					<header class="col-md-12 text-left border-bottom mb-1" style="">
						<!-- Post title-->
						<h1 class="text-black H4" name="board_title"><%=vo.getBoard_title()%></h1>
						<!-- Post meta content-->
						<div class="text-black" name="board_date">
							<%=vo.getBoard_date()%></div>
						<!-- Post categories-->
					</header>
					<div class="md-12" id="blackPage">
						<a class="btn-outline-black btn-sm btn-block" name="mem_id">작성자
							: <%=vo.getMem_id()%></a> <a class="btn-outline-black btn-sm"
							name="board_hit">조회수 : <%=vo.getBoard_hit()%></a>
					</div>
					...
					<div class="row mb-5">
						<div class="col-md-12" style="height: 500px;">
							<section class="border mt-4 p-4 rounded" style="height: 100%;"board_contents">
								<p class="col-12 mb-3 site-blocks-table">
									<%=content%></p>
							</section>
						</div>
					</div>
					...
					<div class="col-md-12 mb-3 mb-md-0">
						<button type="button"
							class="btn btn-black btn-sm btn-block action" data-toggle="modal"
							data-target="#wModal" value="수정" style="margin-top: .6em"
							id="write" data-whatever="@mdo">수정</button>
					</div>
					<div class="col-md-12">
						<input type="button" value="삭제"
							class="btn btn-outline-black btn-sm btn-block action"
							name="delete"><br> <br>
					</div>
					<%
					}
						...
						<div class="card-body pt-4">
							...
							<form class="mb-4">
								<textarea class="form-control" rows="5"
									placeholder="댓글을 입력해 주세요!" name="textarea" style="width: 100%;"></textarea>
								<%
								if (mem_id != null) {
								%>
								<input type="button" value="등록"
									class="btn col-md-12 btn-black action" name="doreply"><br>
								<br>
							</form>
							<%
							}
				...
				<input type="button" value="목록" class="btn btn-outline-black action"
					name="backlist">
				<%
				if (mem_id != null && vo.getBoard_cat_no() != 3) {
				%>
				<input type="button" value="신고"
					class="btn btn-outline-primary action" name="board_repoart"
					id="board_repoart"><br> <br>
				<%
				}
				%>
			</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="mform" name="mform">
						...
						<div class="form-group">
							<label for="board_title" class="col-form-label">제목</label> <input
								type="text" class="form-control txt" id="board_title"
								name="board_title" value="<%=vo.getBoard_title()%>"><br>
						</div>
						...
						<div class="form-group">
							...
							<textarea class="txt" style="width: 100%;" cols="60"
								name="board_contents" id="board_contents"><%=vo.getBoard_contents()%></textarea>
						</div>

						<label for="board_cat_no">게시판종류</label><br> <select
							id="board_cat_no" name="board_cat_no">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" value="전송" id="msend"
						aria-label="Close">전송</button>
				</div>
			</div>
		</div>
	</div>
...
//		    	  $(댓글수정 this).기준으로 reply - body - p3 - append - fcode를
		...
		$('#blackPage').on('click', function(){
		   	 if(mem_id=="admin"){
		   		 ...
		   		 location.href = "<%=request.getContextPath()%>/includes/admin/adminstart.jsp"
		   	 }
