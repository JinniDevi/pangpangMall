   ...
   protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      if(check != null) {
         ...
         if(check.getMem_admin() == 1) {
            response.sendRedirect(request.getContextPath() + "/includes/admin/adminstart.jsp");
         }else {
//         dispatcher = request.getRequestDispatcher("/index.jsp").forward(request, response);
         response.sendRedirect(request.getContextPath() + "/includes/shop/start.jsp");
            }
